...
%          eig_v= eig(Sigma(:,:,j)); 
%          nan_jus= ~isnan(eig_v);
%          min_eig= 2*min(eig_v(nan_jus));
%          temp_eig = eig(Sigma(:,:,j));
%          pertub = 7*max(max(temp_eig)-num_roi*min(temp_eig), 0)/(num_roi-1);
%          Sigma(:,:,j)= Sigma(:,:,j) + pertub*eye(d);
       D = sqdist(data, permute(mu(:,j,:), [1 3 2]), inv(Sigma(:,:,j)))';
       logB2 = -(d/2)*log(2*pi) - 0.5*logdet(Sigma(:,:,j)) - 0.5*D;
       B2(j,:,:) = exp(logB2);
      % error(sprintf('mixgauss_prob: Sigma(:,:,q=%d) not psd\n', j));
       %fprintf('mixgauss_prob: Sigma(:,:,q=%d) not psd\n', j);
     end
