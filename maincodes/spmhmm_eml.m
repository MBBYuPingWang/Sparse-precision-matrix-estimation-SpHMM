function [LL, prior, transmat, mu, Sigma, mixmat] = ...
     spmhmm_eml(data, prior, transmat, mu, Sigma, mixmat, lambda, varargin)
...
% predefined lambda
 lambda_list = O*Q*ones(Q,1);
 InvSgm2 = repmat(eye(O), [1 1 Q]);
while (num_iter <= max_iter) & ~converged
  % E step
  [loglik, exp_num_trans, exp_num_visits1, postmix, m, ip, op] = ...
      ess_mhmm(prior, transmat, mixmat, mu, Sigma, data);
  % cache the objective function of sparse hmm
    loglik = loglik - l1norm(lambda_list, InvSgm2);
    
%    if num_iter > 1 && abs(loglik/previous_loglik) > 100
%        fprintf('Early stop to avoid over-regularization\n');
...
%    end
  ...
  if adj_mu | adj_Sigma
    [mu2, Sigma2, InvSgm2,lambda_list] = mixgaussglasso_Mstepl(postmix, m, op, ip, lambda,'cov_type', cov_type);
     fprintf('lambda_list = %.4f\n', lambda_list(1));
    % fprintf('%.6f < lambdas < %.6f\n',min(lambda_list), max(lambda_list));
    if adj_mu
  ...
  if verbose 
      ...
      % for test 
      % figure, plot_covariance_matrix(Sigma);
  end
  num_iter =  num_iter + 1;
