function [mu, Sigma, InvSgm,lambda_list] = mixgauss_Mstepl(w, Y, YY, YTY, lambda, varargin)
...
% Sigma = Sigma+ cov_prior;  % Here Sigma is the original estimation of covariance matrix, i.e.
%% add the regularization term
  %lambda_vec = sqrt(sum(w)*log(Ysz)/2)./squeeze(Sigma(end,end,:))./w; %\titlde{lambda}
  
  %
  lambda_vec = lambda*sqrt(N*log(Ysz)/2)./w;
  
 % without regularization
 % lambda_vec = 0 * w;
  lambda_list = lambda_vec.*w;%sqrt(sum(w)*log(Ysz)/2)./squeeze(Sigma(end,end,:));
% 
% lambda_vec = sqrt(0.5*log(Ysz)./sum(w))./squeeze(Sigma(end,end,:))
% lambda_vec = 0.1*ones(5,1);
% InvSgm = Sigma;

% InvSgm = Sigma;
%lambda_vec = lambda_vec*N./w*K;
%% for test only
for k=1:Q
%  [Sigma, InvSgm]= KKLglasso(old_sigma,S_corr, gamma_vec, lambda_vec, eta, k); 
% lambda_vec(k) = lambda_vec(k)*N/w(k);
 [Sigma(:,:,k), InvSgm(:,:,k)] = graphlasso(Sigma(:,:,k), lambda_vec(k));%, ...
